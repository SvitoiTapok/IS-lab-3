...
public class ImportBean {
    ...
    @Autowired
    public ImportBean(CitiesRepository citiesRepository, HumanRepository humanRepository, CoordinatesRepository coordinatesRepository, MinioService minioService, ImportResultRepository importResultRepository) {
    ...
    public ResponseEntity<?> importFile(MultipartFile file) {
        ...
        try {
            storedFileName = minioService.saveImportFile(file);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("MINIO connection error");
        }
        try {
            ...
            for (int i = 0; i < objects.size(); i++) {
                ...
                if (objects.get(i).size() != 11) return returnAndSave(HttpStatus.BAD_REQUEST, "Несоответствующее количество полей в записи номер " + (i + 1), storedFileName, originalName);
                ...
                for (String fieldName : currentObj.keySet()) {
                    ...
                    if (currentObj.get(fieldName) instanceof Number) source = numberFieldNames;
                        ...
                        return returnAndSave(HttpStatus.BAD_REQUEST, "Неподдерживаемый формат значения поля " + fieldName + " в объекте под номером " + (i + 1), storedFileName, originalName);
                    if (source.isEmpty())
                        return returnAndSave(HttpStatus.BAD_REQUEST,"Несоответствующее количество полей определенного типа в объекте с номерном " + (i + 1), storedFileName, originalName);
                    double value = -1;
                    ...
                    if (value < 0.7)
                        return returnAndSave(HttpStatus.BAD_REQUEST,"Не найдено совпадений для поля " + fieldName + " в объекте под номером " + (i + 1), storedFileName, originalName);
                    try {
                        currentCity.setFieldByName(mostSimilar, currentObj.get(fieldName));
                    } catch (IllegalArgumentException e) {
                        return returnAndSave(HttpStatus.BAD_REQUEST,"Климат в объекте с номером " + (i+1) + " не предусмотрен системой", storedFileName, originalName);
                    }
                ...
                if (!violations.isEmpty()) {
                    ...
                    return returnAndSave(HttpStatus.BAD_REQUEST,"Ошибка валидации: " + errorMessage, storedFileName, originalName);
                }
                try {
                    int finalI = i;
                    ...
                    return returnAndSave(HttpStatus.NOT_FOUND, e.getMessage(), storedFileName, originalName);
                }
                cities.add(currentCity);
            }
            cities.stream().map(DTOCity::toCity).forEach(citiesRepository::save);
            return returnAndSave(HttpStatus.OK, "Успешно добавлено " + cities.size() + " объектов" + ((responseMessage.isEmpty())?"":(". Список измененных полей: \n" + responseMessage)), storedFileName, originalName);

        }catch (Exception e){
            try {
                minioService.removeFile(storedFileName);
            }catch (Exception e1){
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("MINIO Error");
            }
    ...
    private ResponseEntity<?> returnAndSave(HttpStatus status, String message, String uniqueFileName, String originalFileName) {
